...
func main() {
	client, err := ethclient.Dial("wss://sepolia.infura.io/ws/v3/d36c22d4559d427e821074165c1f07fa")
	if err != nil {
		log.Fatal(err)
	}
	...
	//返回的所有日志将是 ABI 编码，因此它们本身不会非常易读。
	//为了解码日志，我们需要导入我们智能合约的 ABI。
	//为此，我们导入编译好的智能合约 Go 包，它将包含名称格式为 <Contract>ABI 的外部属性。
	//之后，我们使用 go-ethereum 中的 accounts/abi 包的 abi.JSON 函数
	//返回一个我们可以在 Go 应用程序中使用的解析过的 ABI 接口。
	...
	//我们可以使用这些类型创建一个匿名结构体，并将指针作为第一个参数传递给解析后的 ABI 接口的 Unpack 函数，
	//以解码原始的日志数据。
