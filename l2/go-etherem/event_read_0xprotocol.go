...
import (
	...
	exchange "go-etherem/exchange"
)

// 现在在我们的 Go 应用程序中，让我们创建与 0xProtocol 事件日志签名类型匹配的结构体类型：
// LogFill ...
type LogFill struct {
...
func main() {
	...
	contractAbi, err := abi.JSON(strings.NewReader(string(exchange.ExchangeMetaData.ABI)))
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, vLog := range logs {
		...
		case logFillEvent.Hex():
			...
			//Unpack 不会解析 indexed 事件类型，因为这些它们存储在 topics 下，
			//所以对于那些我们必须单独解析，如下例所示：
			var fillEvent LogFill

			_, err := contractAbi.Unpack("LogFill", vLog.Data)
			if err != nil {
				log.Fatal(err)
			}
		...
		case logCancelEvent.Hex():
			...
			_, err := contractAbi.Unpack("LogCancel", vLog.Data)
			if err != nil {
				log.Fatal(err)
			}
