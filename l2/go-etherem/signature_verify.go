...
func main() {
	...
	//现在假设我们有字节格式的签名，我们可以从 go-ethereum crypto 包调用 Ecrecover（椭圆曲线签名恢复）
	//来检索签名者的公钥。
	//此函数采用字节格式的哈希和签名。
	...
	fmt.Println(matches) // true
	//为方便起见，go-ethereum/crypto 包提供了 VerifySignature 函数，该函数接收原始数据的签名，
	//哈希值和字节格式的公钥。 它返回一个布尔值，如果公钥与签名的签名者匹配，则为 true。
