...
func main() {
	...
	// 我们需要弄清楚交易的 data 部分。 这意味着我们需要找出我们将要调用的智能合约函数名，以及函数将接收的输入。
	//然后我们使用函数名的 keccak-256 哈希来检索 方法 ID，它是前 8 个字符（4 个字节）。
	//然后，我们附加我们发送的地址，并附加我们打算转账的代币数量。
	//这些输入需要 256 位长（32 字节）并填充左侧。 方法 ID 不需填充。
	...
	//可以提高gas limit
	//接下来我们需要做的是构建交易事务类型，这类似于您在 ETH 转账部分中看到的，除了_to_字段将是代币智能合约地址。
...
//2024/12/09 21:31:46 insufficient funds for gas * price + value: balance 0, tx cost 1055008708917900, overshot 1055008708917900
//此错误表明在尝试进行区块链交易时，账户余额不足以支付所需的手续费（gas费用）加上转账金额（value）。在这个具体案例中，账户余额为0，而交易的总费用为1055008708917900 wei（以太坊的最小货币单位），因此交易无法执行。
